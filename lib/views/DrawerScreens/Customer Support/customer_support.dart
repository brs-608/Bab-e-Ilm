  ...
  builder: (context, state) {
    ...
    if(state is UserDataState){
      ...
      Future<void> sendMessage(String text, String sender) async {
        await FirebaseFirestore.instance.collection('${state.data['email']}${widget.supportType=="Customer Support"?"_customerSupport":"_techSupport"}').doc(DateTime.now().toString()).set({
          'message': text,
          'sender': sender,
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection("${state.data['email']}${widget.supportType == "Customer Support"?"_customerSupport":"_techSupport"}").orderBy("timestamp").snapshots(),
                            ...
                            RichText(text: TextSpan(
                              children: [
                                ...
                                WidgetSpan(child: Padding(
                                  ...
                                  child: Icon(Icons.verified,size: 14,color: Colors.yellow,),
                                ))
                              ]
                            )):
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: Form(
                              child: TextFormField(
                                style: english.hasMatch(messageController.text.toString()) == false?TextStyle(
                                    fontFamily: "jameel"
                                ):GoogleFonts.roboto(),
                          ...
                          IconButton(onPressed: ()async{
                              await sendMessage(messageController.text.toString(), userName);
